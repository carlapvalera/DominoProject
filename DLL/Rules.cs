...
public class Action : IAction// implemetacion de la interface
{
    ...
    public List<Piece> ToSub(List<Piece> hand,Table table)
    {
        List<Piece> aux = new();
        bool change;
        ...
        do
        {
            ...
            while (change)
                Change(hand, aux, count,table);

        } while (Double(hand).Count > 0);
        return hand;
    }
    private List<Piece> Change(List<Piece> hand, List<Piece> aux, int n,Table table)//cambiar los dobles envontrados en la mano del jugador por nuevas fichas
    {
        ...
        list = NewDistribute(hand, n,table);
        foreach (Piece piece in aux)
        {
    ...
    private List<Piece> NewDistribute(List<Piece> hand, int n,Table table)//volver a repartirle fichas al jugador hasta la cantidad deseada
    {
